...
namespace Memcury
{
    ...
    public:
        ...
        static auto FindStringRef(T string, bool find_first = false) -> Scanner
        {
            ...
            const auto textSectionSize = textSection.GetSectionSize();
            const auto rdataStart = rdataSection.GetSectionStart().Get();
            const auto rdataEnd = rdataStart + rdataSection.GetSectionSize();

            // scan only text section
            for (DWORD i = 0x0; i < textSectionSize; i++)
            {
                const std::uint8_t byte = scanBytes[i];
                if ((byte == ASM::CMOVL || byte == ASM::CMOVS) && scanBytes[i + 1] == ASM::LEA)
                {
                    auto stringAdd = PE::Address(&scanBytes[i]).RelativeOffset(3);

                    // Check if the string address is within the .rdata section
                    if (rdataStart <= stringAdd.Get() && stringAdd.Get() < rdataEnd)
                    {
